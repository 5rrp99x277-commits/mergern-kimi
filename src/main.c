...
static bool handle_dl_control_common(uint8_t rhport, tusb_control_request_t const *request) {
    ...
    if (cmd == DL_GETVER) {
        ...
        return tud_control_xfer(rhport, request, ctrl_buf, sizeof(*id));
    }
    ...
    if (cmd == DL_RESETCFG) {
        rdl_state_t *st = (rdl_state_t *)ctrl_buf;
        st->state = DL_RUNNABLE;
        st->bytes = fw_bytes_received;
        printf("%s DL_RESETCFG ACK\r\n", boot_mode ? "BOOT" : "RUNTIME/POSTBOOT");
        return tud_control_xfer(rhport, request, ctrl_buf, sizeof(*st));
    }
...
static uint16_t prepare_bcdc_response(uint8_t *buf, uint16_t max_len) {
    ...
    const char *iovar = last_iovar_valid ? last_iovar_name
                        : (const char *)(bcdc_buf + sizeof(bcdc_hdr_t));
    ...
    } else if (effective_cmd == BRCMF_C_GET_REVINFO) {
        payload_len = last_bcdc_len ? last_bcdc_len : 68;
        if (payload_len < 68 && max_payload >= 68) payload_len = 68;
        if (payload_len > max_payload) payload_len = max_payload;
        memset(payload, 0, payload_len);
...
static bool parse_runtime_out(uint16_t total) {
    ...
    if (!is_supported_bcdc_cmd(hdr->cmd)) {
    ...
    // Save iovar name for prepare_bcdc_response (survives if ctrl_buf/bcdc_buf get touched)
    ...
    // Detect cmd=0 wrapper: if cmd==0 but embedded payload has real command, we still
    // want to parse the iovar. effective_cmd will be resolved in prepare_bcdc_response.
...
bool tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const * request) {
    ...
    // Handle vendor-IN DL commands in both boot and runtime/postboot modes.
    if (handle_dl_control_common(rhport, request)) {
        return true;
    }
    ...
    if (!boot_mode) {
        if (stage == CONTROL_STAGE_SETUP) {
            ...
            if (request->bmRequestType == 0xA1 && request->bRequest == 1) {
                ...
                const char *name = safe_iovar_name(bcdc_buf + sizeof(bcdc_hdr_t), sizeof(bcdc_buf) - sizeof(bcdc_hdr_t));
